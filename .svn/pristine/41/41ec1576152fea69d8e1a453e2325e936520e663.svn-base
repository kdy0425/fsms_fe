...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/ilg/mlee/tx/getAllMot72LiterExcsElctcList?page=${params.page - 1}&size=${params.size}` +
  ...
  return (
    <PageContainer title="1회 72리터 초과충전" description="1회 72리터 초과충전">
      {/* breadcrumb */}
      <Breadcrumb title="1회 72리터 초과충전" items={BCrumb} />
      {/* end breadcrumb */}
