...
const BsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/vpm/bs/getAllVhclePauseMng?page=${params.page - 1}&size=${params.size}` +
        `${params.sort ? '&sort=' + sortChange(params.sort) : ''}` +
        `${!dtFlag && params.searchStDate ? '&bgngYmd=' + params.searchStDate : ''}` +
        `${!dtFlag && params.searchEdDate ? '&endYmd=' + params.searchEdDate : ''}`+
