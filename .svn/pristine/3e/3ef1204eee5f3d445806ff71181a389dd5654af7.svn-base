...
const DataList = () => {
  ...
  const getDateRange = () => {
    ...
    before1Month.setMonth(today.getMonth() - 1);
    
    let stYear:string = String(before1Month.getFullYear());
    let stMonth:string = String(before1Month.getMonth()+1).padStart(2, '0'); // yyyy-mm-dd 형식을 위해 한자리 숫자 월,일에 0붙이기

    let edYear = String(today.getFullYear());
    let edMonth = String(today.getMonth()+1).padStart(2, '0');

    const startDate = stYear+"-"+stMonth;
    const endDate = edYear+"-"+edMonth;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/apv/bmdd/bs/getAllBusMnbyDelngDtls?page=${params.page - 1}&size=${params.size}` +
  ...
  const fetchDetailData = async () => {
    try {
      let endpoint = `/fsm/apv/bmdd/bs/getAllBusDelngDtls?page=${detailParams.page - 1}&size=${detailParams.size}` +
  ...
  const excelDownload = async () => {
    try {
      let endpoint: string =
      `/fsm/apv/bmdd/bs/getExcelBusMnbyDelngDtls?page=${params.page - 1}&size=${params.size}` +
      `${params.sort ? '&sort=' + sortChange(params.sort) : ''}` +
      `${params.searchStDate ? '&bgngDt=' + params.searchStDate.replaceAll("-", "") : ''}` +
      `${params.searchEdDate ? '&endDt=' + params.searchEdDate.replaceAll("-", "") : ''}` + 
      `${!isLocgovCdAll && params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}`+
