...
const DataList = () => {
  ...
  const [vhclSeCdItems, setVhclSeCdItems] = useState<SelectItem[]>([]);
  ...
  useEffect(() => {
    ...
    getCommCd('505', '전체').then((itemArr) => setVhclSeCdItems(itemArr)); // 면허업종 코드
  }, [])
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/cal/byvsr/bs/getAllByveSbsidyRqest?page=${params.page - 1}&size=${params.size}` +
