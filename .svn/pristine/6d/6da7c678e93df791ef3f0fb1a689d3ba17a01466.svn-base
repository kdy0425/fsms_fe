...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/dqi/tr/ddpsQualfInfo?page=${params.page - 1}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    let endpoint: string =
      `/fsm/stn/dqi/tr/getExcelDdpsQualfInfoTr?page=${params.page - 1}&size=${params.size}` +
        `${params.sort ? '&sort=' + sortChange(params.sort) : ''}` +
        `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
        `${params.vhclNo ? '&vhclNo=' + params.vhclNo : ''}` +
        `${params.vonrRrno ? '&vhclNo=' + params.vonrRrno : ''}` 
      
    getExcelFile(endpoint, '' + '운수종사자격정보'+getToday()+'.xlsx')
  }
  ...
  const excelHisDownload = async () => {
    if(selectedRow == undefined){
      ...
      let endpoint: string =
      `/fsm/stn/dqi/tr/getExcelDdpsQualfHstTr?` +
        `${selectedRow.vonrRrno ? '&vonrRrno=' + selectedRow.vonrRrno : ''}` 

      getExcelFile(endpoint, '' + '운수종사자격정보_이력'+getToday()+'.xlsx')
  }
  ...
  return (
    <PageContainer title="화물 운수종사자격정보" description="차량휴지관리 페이지">
      {/* breadcrumb */}
