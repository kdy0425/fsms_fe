...
const DetailPage = () => {
  ...
  const getDetailData =  async (prgrmNm: string) => {
    try {
      let endpoint: string = `/fsm/sym/progrm/cm/getAllProgrm?prgrmNm=${prgrmNm}`;
      ...
      if(response && response.resultType === 'success' && response.data) {
        const data = response.data.content.find((data:Row) => data.prgrmCdSn === querys.get("prgrmCdSn"));
        console.log("DATA ::: ", data);
        setParams({
          prgrmCdSn: data["prgrmCdSn"] ?? "",
          prgrmNm: data["prgrmNm"] ?? "",
          urlAddr: data["urlAddr"] ?? "",
          httpDmndMethNm: data["httpDmndMethNm"] ?? "",
          useYn: data["useYn"] ?? "",
  ...
  useEffect(() => {
    const prgrmNm = String(querys.get("prgrmNm"));
      ...
      <Box>
        ...
        <Box className="table-bottom-button-group">
          <div className="button-right-align">
            <Button variant="contained" color="primary" onClick={updateProgrm}>저장</Button>
            <Button variant="contained" color="primary" onClick={() => router.push("/sym/progrm")}>취소</Button>
          </div>
        </Box>
      </Box>
    </PageContainer>
