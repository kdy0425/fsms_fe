...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/apv/mdd/cm/getAllMnbyDelngDtls?page=${params.page - 1}&size=${params.size}` +
  ...
  const fetchDetailData = async () => {
    ...
    try {
      let endpoint = `/fsm/apv/mdd/cm/getAllDelngDtls?page=${detailParams.page - 1}&size=${detailParams.size}` +
