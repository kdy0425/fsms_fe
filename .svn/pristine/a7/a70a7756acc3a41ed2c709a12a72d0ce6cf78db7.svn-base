...
const DataList = () => {
  ...
  const [ctpvCdItems, setCptvCdItems] = useState<SelectItem[]>([]) // 시도 코드
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/cal/bsnsr/tx/getAllBsnmSbsidyRqest?page=${params.page - 1}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    let endpoint: string =
        `/fsm/cal/bsnsr/tx/getExcelAllBsnmSbsidyRqest?`+
        `${params.locgovCd ? '&clclnLocgovCd=' + params.locgovCd : ''}` + 
        `${params.searchStDate ? '&bgngDt=' + formatDate(params.searchStDate) : ''}` + 
        `${params.searchEdDate ? 'endDt=' + formatDate(params.searchEdDate) : ''}`
