...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sym/ucdh/cm/getAllUnConectrDelHist?page=${params.page - 1}&size=${params.size}` +
  ...
  // 행 클릭 시 호출되는 함수
  const handleRowClick = async (delYmd: string) => {
    try {
       let endpoint: string = `/fsm/sym/ucdh/cm/getAllUnConectrDelHistDtl?&delYmd=${delYmd}`;
  ...
  const setDateRange = () => { // 기본 검색 날짜 범위 설정 (30일)
