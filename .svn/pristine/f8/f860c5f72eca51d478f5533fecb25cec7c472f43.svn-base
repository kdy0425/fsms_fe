...
const DetailDataGrid: React.FC<DetailDataGridProps> = ({ 
  ...
  return (
    ...
    <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
      ...
      <BlankCard title="운행정지정보 변경이력">
    <div className="data-grid-wrapper">
        <div className="table-scrollable">
          <table className="table table-bordered">
          <TableHead>
              <TableRow>
                <TableCell colSpan={8}>운행정지정보 변경이력</TableCell>{/* 합계 헤더 */}
              </TableRow>
              <TableRow>
                <TableCell>순번</TableCell>
                <TableCell>차량번호</TableCell>
                <TableCell>입력구분</TableCell>
                <TableCell>운행정지등록구분</TableCell>
                <TableCell>운행정지등록사유</TableCell>
                <TableCell>운행정지등록/해제일자</TableCell>
              ...
              {loading ? (
                ...
                rows.map((row, index) => (
                  ...
                  >
                    ...
                    <TableCell>{row.oprStopRegSeCdNm ?? ''}</TableCell>
                    <TableCell>{row.oprStopRegSttsCdNm ?? ''}</TableCell>
                    <TableCell>{row.stopRsnCn ?? ''}</TableCell>
                    <TableCell>{formatDate(row.oprStopRegRmvYmd) ?? ''}</TableCell>
