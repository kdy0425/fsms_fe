  ...
  return (
    <div className="data-grid-top-toolbar">
      <div className="data-grid-search-count">
        검색 결과 <span className="search-count">{getCommaNumber(props.totalRows)}</span>건
      </div>
    </div>
  )
}
  ...
  function getCellValue(row:any, headCell:any) {
    try{
      if(headCell.format === 'brno'){
        ...
        return String(row[headCell.id]).trim().length > 0 ? dateTimeFormatter(row[headCell.id]) : ''
        ...
        <TableContainer>
          ...
          >
            ...
            {!loading ? 
                              ...
                              (
                                (headCell.rowspan && isRowspan(index, headCell.id)) || isColspan(row, headCells, i) ? 
                                  ...
                                  <TableCell 
                                    ...
                                    style={{color: (headCell.color ? row['color'] : 'black'), whiteSpace:'nowrap', textAlign: (headCell.format == 'lit' || headCell.format == 'number' ? 'right' : 'center')}}>
              ...
              : 
              <TableRow key={'tr0'}>
                <TableCell colSpan={headCells.length} className='td-center'><p><CircularProgress /></p></TableCell>
              </TableRow>
            }
            </TableBody>
          </Table>
