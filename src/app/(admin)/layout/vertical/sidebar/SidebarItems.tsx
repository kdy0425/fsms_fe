...
const SidebarItems = () => {
  ...
  const fetchData = async () => {
    try {
      const endpoint = `/fsm/sym/fmm/cm/getAllFsmMenu`;
      const response = await sendHttpRequest('GET', endpoint, null, true, {
        cache: 'no-store',
      });
      if (response && response.resultType === 'success' && response.data) {
        const rootMenus = response.data.filter((menu: Menu) => menu.menuNm === 'ROOT')[0];
        const mappedData = rootMenus.children.map((menu: Menu) => convertRowToMenuitemsType(menu));
