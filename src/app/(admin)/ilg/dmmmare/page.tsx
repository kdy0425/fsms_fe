...
import { brNoFormatter, getDateRange, getDateTimeString } from '@/utils/fsms/common/util'
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/ilg/dmmmare/tx/getAllDay4DoubtReptList?page=${params.page - 1}&size=${params.size}` +
  ...
  const fetchDetailData = async () => {
    try {
      // 검색 조건에 맞는 endpoint 생성
      let endpoint: string =
        `/fsm/ilg/dmmmare/tx/getAllDay4DoubtReptDtlList?page=${detailParams.page - 1}&size=${detailParams.size}` +
        `${'&dlngYm=' + detailParams.dlngYm}` +
        `${'&locgovCd=' + detailParams.locgovCd}` +
        `${'&brno=' + detailParams.brno}` +
        `${'&vhclNo=' + detailParams.vhclNo}` +
        `${'&bzmnSeCd=' + detailParams.bzmnSeCd}` +
  ...
  return (
    <PageContainer title="1일4회이내 월4회이상반복충전" description="1일4회이내 월4회이상반복충전">
      {/* breadcrumb */}
      <Breadcrumb title="1일4회이내 월4회이상반복충전" items={BCrumb} />
      {/* end breadcrumb */}
