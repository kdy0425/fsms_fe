...
interface ServerPaginationGridProps {
  ...
  onRowClick: (index: number) => void // 행 클릭 핸들러 추가
          ...
          >
            ...
            <TableBody>
              {!loading ? (
                type == 'g' && rows.length > 0 ? (
                  rows.map((row: any, index) => {
                    return (
                      ...
                      >
                        <TableCell>{row.cdGroupNm}</TableCell>
                        <TableCell>{row.cdKornNm}</TableCell>
                        <TableCell>{row.cdSeNm}</TableCell>
                        <TableCell>{row.cdExpln}</TableCell>
                        <TableCell>{row.useNm}</TableCell>
                        <TableCell>{row.comCdYn}</TableCell>
                        <TableCell>
                          <FormModal
                ...
                type == 'c' && rows !== undefined && rows.length > 0 ? (
                  rows.map((row: any, index) => {
                    return (
                      <TableRow
                        key={'tr' + index}
                      >
                        <TableCell>{row.cdGroupNm}</TableCell>
                        <TableCell>{row.cdNm}</TableCell>
                        <TableCell>{row.cdKornNm}</TableCell>
                        <TableCell>{row.cdExpln}</TableCell>
                        <TableCell>{row.cdSeq}</TableCell>
                        <TableCell>{row.useYn == 'Y'? '사용' : '미사용'}</TableCell>
                        <TableCell>
                        <FormModal
                          reloadFunc={reloadFunc}
                          buttonLabel='수정'
