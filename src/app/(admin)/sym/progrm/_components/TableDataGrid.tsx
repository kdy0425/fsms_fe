...
interface ServerPaginationGridProps {
  ...
  onRowClick: (prgrmCdSn:string, prgrmNm:string) => any // 행 클릭 핸들러 추가
  onCheckItem: (selected: readonly string[]) => any
            ...
            <TableBody>
              {!loading ? 
                rows.length > 0 ? rows.map((row: any, index) => {
                  const isItemSelected = isSelected(row.prgrmCdSn)
                  const labelId = `enhanced-table-checkbox-${index}`

                  return (
                    <TableRow 
                      hover 
                      onClick={() => onRowClick(row.prgrmCdSn, row.prgrmNm)}
                      role="checkbox"
                    ...
                    >
                      <TableCell padding="checkbox">
                        <CustomCheckbox
                          onClick={(event) => handleClick(event, row.prgrmCdSn)}
                          name='prgrmCdSn'
