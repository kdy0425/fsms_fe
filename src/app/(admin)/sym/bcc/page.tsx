...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sym/bcc/bs/getAllBusCardCntc?page=${params.page - 1}&size=${params.size}` +
  ...
  const fetchHistoryData = async (procNm:string) => {
    // 이력 데이터 호출
    let endpoint : string = `/fsm/sym/bcc/bs/getAllBusCardCntcHst?procNm=${procNm}`;
  ...
  const reExcute = async (procNm:string, event: React.MouseEvent<unknown>) => {
    ...
    let endpoint : string = `/fsm/sym/bcc/bs/excuteBusCardCntc?procNm=${procNm}`;
