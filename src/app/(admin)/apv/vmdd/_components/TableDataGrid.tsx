...
export interface ServerPaginationGridProps {
  headCells: HeadCell[]
  rows: Row[] // 목록 데이터
              ...
              {!loading ? (
                rows.length > 0 ? (
                  rows.map((row: any, index) => {
                    return (
                      <TableRow key={'tr' + index} onDoubleClick={() => onRowClick(row)} hover>
                        <TableCell>{row.vhclNo}</TableCell>
                        <TableCell>{getDateTimeString(row.dlngYm, "date")?.dateString.slice(0,-1)}</TableCell>
                        ...
                        <TableCell style={{whiteSpace:'nowrap', textAlign:'right',}}>
                          {row.asstAmt.toLocaleString('ko-KR')}</TableCell>
                        <TableCell style={{whiteSpace:'nowrap', textAlign:'right',}}>
                          {row.ftxAsstAmt.toLocaleString('ko-KR')}</TableCell>
                        <TableCell style={{whiteSpace:'nowrap', textAlign:'right',}}>
                          {row.opisAmt.toLocaleString('ko-KR')}</TableCell>
                  ...
                  <TableRow key={'tr0'}>
                    <TableCell colSpan={11} className="td-center">
