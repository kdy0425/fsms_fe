...
const DataList = () => {
  ...
  const detailhandlePaginationModelChange = (page: number, pageSize: number) => {
    setDetailParams((prev) => ({
      ...
      // 실제 DB에서 조회시 -1을 하므로 +1 추가해서 넘겨야함.
      // 페이지는 1로 보이지만 DB에선 0으로 조회
      size: pageSize,
    }))
  }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/disi/tr/dutyInsrSbscInfo?page=${params.page - 1}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    let endpoint: string =
      `/fsm/stn/disi/tr/dutyInsrSbscInfo?page=${params.page - 1}&size=${params.size}` +
        ...
        `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
        `${params.vhclNo ? '&vhclNo=' + params.vhclNo : ''}` 
    getExcelFile(endpoint, '' + BCrumb[BCrumb.length - 1].title+getToday()+'.xlsx')
  }
  ...
  const fetchhistoryData = async () => {
    ...
    try {
      // 검색 조건에 맞는 endpoint 생성
      let endpoint: string =
        `/fsm/stn/disi/tr/dutyInsrSbscHst?page=${detailParams.page - 1}&size=${detailParams.size}` +
        `${detailParams.sort ? '&sort=' + sortChange(params.sort) : ''}` +
  ...
  const excelHisDownload = async () => {
    if(selectedRow == undefined){
      ...
      let endpoint: string =
      `/fsm/stn/disi/tr/getExcelDutyInsrSbscHstTr?page=${params.page - 1}&size=${params.size}` +
        `${params.sort ? '&sort=' + sortChange(params.sort) : ''}` +
        `${selectedRow.locgovCd ? '&locgovCd=' + selectedRow.locgovCd : ''}` +
        `${selectedRow.vhclNo ? '&vhclNo=' + selectedRow.vhclNo : ''}` 

      getExcelFile(endpoint, '' + '의무보험가입정보_이력'+getToday()+'.xlsx')
  }
  ...
  return (
    <PageContainer title="화물 의무보험가입정보" description="차량휴지관리 페이지">
      {/* breadcrumb */}
