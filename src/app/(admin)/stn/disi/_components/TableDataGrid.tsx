    ...
    <ThemeProvider theme={themeWithLocale}>
      <div className="data-grid-wrapper">
        ...
        <div className="table-scrollable">
          ...
          <TableHead>
              <TableRow>
                <TableCell style={{whiteSpace:'nowrap'}} colSpan={5}>차량정보</TableCell>{/* 합계 헤더 */}
                <TableCell style={{whiteSpace:'nowrap'}} colSpan={12}>의무보험 가입정보</TableCell>{/* 합계 헤더 */}
              </TableRow>
              <TableRow>
                <TableCell style={{whiteSpace:'nowrap'}} >차랑변호</TableCell>
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell style={{whiteSpace:'nowrap'}} colSpan={17} className="td-center">
                    <p>로딩 중입니다...</p>
                  </TableCell>
                </TableRow>
              ) : rows.length > 0 ? (
                rows.map((row, index) => (
                  ...
                  >
                    <TableCell className='td-center'>{row.vhclNo}</TableCell>
                    <TableCell className='td-center'>{row.vonrNm}</TableCell>
                    <TableCell className='td-center'>{row.carOwnNm}</TableCell>
                    <TableCell className='td-center'>{row.carSts}</TableCell>
                    <TableCell className='td-center'>{formatDate(row.carRegDt)}</TableCell>

                    <TableCell className='td-center'>{row.twdpsn1SeCdNm}</TableCell>
                    <TableCell className='td-center'>{formatDate(row.twdpsn1EraYmd)}</TableCell>
                    <TableCell className='td-center'>{formatDate(row.twdpsn1EotYmd ?? '')}</TableCell>
                    <TableCell className='td-center'>{row.twdpsn2SeCdNm}</TableCell>
                    <TableCell className='td-center'>{formatDate(row.twdpsn2EraYmd)}</TableCell>
                    <TableCell className='td-center'>{formatDate(row.twdpsn2EotYmd ?? '')}</TableCell>

                    <TableCell className='td-center'>{row.sbsttCdNm}</TableCell>
                    <TableCell className='td-center'>{formatDate(row.sbsttEraYmd)}</TableCell>
                    <TableCell className='td-center'>{formatDate(row.sbsttEotYmd)}</TableCell>
                    <TableCell className='td-center'>{row.insrncCoCdNm}</TableCell>
