...
export const getNumtoWonAndDecimalPoint = (value: number | string | undefined): string =>{
    if  (value === undefined || value === null) {
        return '';
    }

    // 문자열로 변환 후 모든 쉼표 제거
    const cleanedValueStr = typeof value === 'string' ? value.replace(/,/g, '') : value.toString();
...
export const getLabelFromCode = (data: SelectItem[])  => (value: string): string => {
...
export const formatDate = (dateStr: string | undefined): string => {
    if (dateStr === '' || dateStr == undefined || dateStr.length === undefined) return ''; // 빈 문자열 처리
...
export const formatToTwoDecimalPlaces = (value: number | string | undefined): string => {
    if (value === undefined || value === null) {
        return '';
    }
    ...
    // 숫자 형식이 아닌 경우 그대로 반환
    if (isNaN(Number(cleanedValueStr)) || !/^-?\d*\.?\d*$/.test(cleanedValueStr)) {
        return typeof value === 'string' ? value : value.toString();
    }
